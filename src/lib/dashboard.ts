...
export interface DashboardPayment { clientId: string; amount: number; date: string }
...
export async function fetchDashboardData(trainerId: string) {
  ...
  const { data: payments } = clientIds.length
    ? await supabase.from("client_payments").select("client_id,amount,date").in("client_id", clientIds)
    : { data: [] as any[] };
  return {
    ...
    payments: (payments ?? []).map((p) => ({ clientId: p.client_id, amount: Number(p.amount) || 0, date: p.date } as DashboardPayment)),
  };
}
