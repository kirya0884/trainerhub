    ...
    <div className="space-y-4">
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 space-y-2 overflow-hidden">
        <div className="grid grid-cols-[auto_1fr_auto_1fr] items-center gap-1.5">
          ...
          <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="min-w-0 bg-zinc-800 rounded-lg px-2 py-1 text-sm outline-none focus:ring-1 focus:ring-lime-400/40 w-full" />
          <span className="text-zinc-600 text-center">—</span>
          <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="min-w-0 bg-zinc-800 rounded-lg px-2 py-1 text-sm outline-none focus:ring-1 focus:ring-lime-400/40 w-full" />
        </div>
